...
class WarriorController extends Controller
{
    ...
    public function loginCheck(Request $request)
    {
        ...
        if (filter_var($login, FILTER_VALIDATE_EMAIL)) {
            if (Warrior::where('email', $login)->exists()) {
                $loginCheck = Warrior::select('*')->where('email', $login)->first();
                $hashedPassword = $loginCheck->password;
                
                if (Hash::check($request->password, $hashedPassword)){
             
                    $result = Warrior::where('email', $login)->update([
                    ...
                    $userDetails = [
                        ...
                        'token' => $token,
                        'role_category' => $loginCheck->role_category,
            ...
            if (strlen($phoneNumber) >= 10) {
                if (Warrior::where('phone', $login)->exists()) {
                    $loginCheck = Warrior::select('*')->where('phone', $login)->first();
                    $hashedPassword = $loginCheck->password;
                    
                    if (Hash::check($request->password, $hashedPassword)){
                 
                        $result = Warrior::where('phone', $login)->update([
                            'remember_token' => $token
                        ]);
                        $userDetails = [
                ...
                if (Warrior::where('username', $login)->exists()) {
                    $loginCheck = Warrior::select('*')->where('username', $login)->first();
                    $hashedPassword = $loginCheck->password;
                    
                    if (Hash::check($request->password, $hashedPassword)){
                 
                        $result = Warrior::where('username', $login)->update([
                            'remember_token' => $token
                        ]);
                        $userDetails = [
                            'email' => $loginCheck->email,
    ...
    //logout 
    public function logout( $warrior_id){

        if(Warrior::where('id',  $warrior_id)->exists()){
            
            $result = Warrior::where('id',  $warrior_id)->update([
                    'remember_token' => null
            ]);
    ...
    //update profile
    public function updateProfile(Request $request, $warrior_id){
        $validator = Validator::make($request->all(), [
        ...
        if($DB_token == $user_access_token){
             $UserCheck = Warrior::select('*')->where('id', $warrior_id)->first();
            ...
            if($UserCheck != null) {
                ...
                $linkedlin_link = $request->linkedlin_link ? $request->linkedlin_link:$UserCheck->linkedlin_link;
                 ...
                 $update = Warrior::where('id', $warrior_id)->update([
                             'first_name' => $first_name,
                             'last_name' => $last_name,
                             'address1' => $address1,
                             'address2' => $address2,
    ...
    //Update profile pic
    public function UpdateProfilePic(Request $request, $warrior_id){
        ...
        if(Warrior::where('id', $warrior_id)->exists()){
            ...
            if($DB_token == $user_access_token){
                ...
                $userDetails = Warrior::select('profile_pic', 'profile_path')->where('id', $warrior_id)->first();
                ...
                if ($request->hasFile('profic_pic')) {
                    ...
                    $fileModel = Warrior::where('id', $warrior_id)->update([
    ...
    // Delete profile pic
    public function RemoveProfilePic(Request $request, $warrior_id){
        ...
        if(Warrior::where('id', $warrior_id)->exists()){
            ...
            if($DB_token == $user_access_token){
                ...
                $userDetails = Warrior::where('id', $warrior_id)->update([
                    $profile_pic = null,
                    $profile_path = null
                ]);
    ...
    //Change Password
    public function changePassword(Request $request, $warrior_id){
        if(Warrior::where('id', $warrior_id)->exists()){
            $user_access_token  = $request->token;
            ...
            if($DB_token == $user_access_token){
                ...
                $user_password = Warrior::select('password')->where('id', $warrior_id)->first();
    ...
    //Profile view
    public function profileView(Request $request, $warrior_id){
        if(Warrior::where('id', $warrior_id)->exists()){
            $user_access_token  = $request->token;
            $TokenCheck = Warrior::where('id', $warrior_id)->first();
            $DB_token = $TokenCheck->remember_token;
            if($DB_token == $user_access_token){
                $profileData = Warrior::leftjoin('role_categories','role_categories.id','=','warriors.role_category')
                                        ->where('warriors.id',$warrior_id)
                                        ->select('warriors.first_name', 'warriors.last_name', 'warriors.email', 'warriors.phone','warriors.username',
                                        'role_categories.name as role_category','warriors.address1', 'warriors.address2', 'warriors.city','warriors.district',
                                        'warriors.state','warriors.country','warriors.pincode','warriors.job','warriors.company',
                                        'warriors.about','warriors.twitter_link','warriors.facebook_link','warriors.instagram_link','warriors.linkedlin_link')
                                        ->get();

                $profileImage = Warrior::select('id', 'profile_pic')->where('id', $warrior_id)->get();
    ...
    {
        $fileDetails = Warrior::find($fileId);
